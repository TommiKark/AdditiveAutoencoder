function out = DsLbfgsc1(FUN,x0,X,FoldLabels,varargin)
...
%
% Based on another modification of the modified Poblano's Lbfgs routine
% in Lbfgsc1.m by T. Karkkainen (April, 2020).
...
xk = x0;
[fk,gk] = feval(FUN,xk,X);

out = poblano_out(xk,fk,gk,1,params);

% Number of folds (add by TommiK)
nfolds = max(FoldLabels);

% Tolerance to restart limited memory updates when inexact line search
% creates problems for the parameters (square root of machine eps)
ResTol = sqrt(eps);
UpIters = 0;
xkold = -x0;
gkold = -gk;
...
while out.ExitFlag == -1
    ...
    if out.Iters == 0
        ...
        % NOTE: Here danger when the solution or gradient is not changed 
        % because of the inexact line search -> skyk ~ ykyk ~ 0
        UpIters = UpIters + 1;
        skyk = yk'*sk;
        ykyk = yk'*yk;
        rhok = 1 / max(ResTol,skyk);
        gamma = skyk/max(ResTol,ykyk);
        if UpIters <= params.Results.M
            S = [sk S];
            Y = [yk Y];
            rho = [rhok rho];
        else
    ...
    % Select random fold to speed up the line search
    XF = X(FoldLabels == randi(nfolds),:);

    % Use cost function value with fold data
    [fk,gk] = feval(FUN,xk,XF);
    [xk,fk,gk,ak,lsinfo,nfev] = cvsrchinc(FUN,xk,XF,fk,gk,ak,pk,params.Results);
...
function [x,f,g,stp,info,nfev] = cvsrchinc(fcn,x,X,f,g,stp,s,params)
...
%
dginit = g'*s;
% Omit the check below due to inexact optimization scenario.
% if (dginit >= 0.0)
%     fprintf('Not a descent direction in cvsrchinc. Returning with no iterations.\n');
%     return
% end
%
